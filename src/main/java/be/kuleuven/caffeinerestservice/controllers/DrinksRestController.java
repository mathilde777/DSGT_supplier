...
public class DrinksRestController {
    ...
    @Autowired
    DrinksRestController(DrinksRepository drinksRepository) {this.drinksRepository = drinksRepository;}
    ...
    EntityModel<Drink> getDrink(@PathVariable String id, @PathVariable String code) {
        if(checkCode(code)) {
            ...
            return drinkToEntityModel(id, d, "1234");
        }
        else{
            throw new CodeNotCorrectException(code);
        }
    ...
    @GetMapping("/rest/stockID/{id}/{code}")
    ...
    @PostMapping("/rest/drinksId/{id}/reserve/{reservationId}/{code}")
    public ResponseEntity<String> reserveDrink(@PathVariable String id, @PathVariable String reservationId, @PathVariable String code) {
        if(checkCode(code)) {
            // Assuming checkCode verifies the correctness of the code
            drinksRepository.reserveDrink(id, reservationId); // Reserve the drink
            ...
            // Throw an exception if the code is not correct
            throw new CodeNotCorrectException(code);
        }
    }
    ...
    public ResponseEntity<String> buyDrink(@PathVariable String reservationId, @PathVariable String code) {
        if (checkCode(code)) {
            ...
            if (success) {
                return ResponseEntity.ok("Drinks with reservation ID " + reservationId + " purchased successfully.");
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Failed to purchase drinks with reservation ID " + reservationId);
            }
        } else {
            throw new CodeNotCorrectException(code);
        }
